...
from utils import cos_similarity_score, imgs_resize
...
def L1_projection(x2, y2, eps1):
    ...
    x = x2.clone().float().reshape(x2.shape[0], -1)
    y = y2.clone().float().reshape(y2.shape[0], -1)
    sigma = y.clone().sign()
...
class APGDAttack():
    ...
    def normalize(self, x):
        if self.norm == 'Linf':
            ...
            t = (x ** 2).reshape(x.shape[0], -1).sum(-1).sqrt()
    ...
    def fr_loss_targeted(self, x, y):
        
        return cos_similarity_score(x,y)
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if not self.is_tf_model:
            ...
            elif self.loss == 'fr_loss_targeted':
                criterion_indiv = self.fr_loss_targeted
            else:
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    x_adv_r = reconstruct(x_adv)
                    x_adv_r = imgs_resize(x_adv_r,self.shape)
                    logits = self.model(x_adv_r)
        ...
        acc = cos_similarity_score(logits,y)<self.thres
        acc_steps[0] = acc + 0
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    grad_topk = grad.abs().reshape(x.shape[0], -1).sort(-1)[0]
            ...
            pred = cos_similarity_score(logits,y)<self.thres
        ...
        n_query = self.eot_iter + (i+1) * self.eot_iter

        return (x_best, acc, loss_best, x_best_adv), n_query

    def perturb(self, x, y=None, best_loss=True, x_init=None):
        """
        ...
        if not self.is_tf_model:
            x_r = reconstruct(x)
            x_r = imgs_resize(x_r,self.shape)
            y_pred = self.model(x_r)
        else:
        ...
        if self.loss == 'fr_loss_targeted':
            print(f"Initial Loss = {cos_similarity_score(y_pred,y).detach().cpu().numpy()}")
            acc = cos_similarity_score(y_pred,y)<self.thres
        else:
            raise ValueError('cannot attack FR')
        if self.verbose:
            print('-------------------------- ',
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.use_largereps:
                        res_curr, n_query = self.attack_single_run(x_to_fool, y_to_fool)
                    else:
                    ...
                    if self.verbose:
                        print('restart {} - robust accuracy: {:.2%}'.format(
                            ...
                            time.time() - startt))
            print(f"Final Loss = {loss_curr.detach().cpu().numpy()}")

            return adv, n_query, loss_curr
            ...
            for counter in range(self.n_restarts):
                ...
                res_curr, n_query = self.attack_single_run(x, y)
            ...
            print(f"Final Loss = {loss_best.detach().cpu().numpy()}")

            return adv_best, n_query, loss_best
